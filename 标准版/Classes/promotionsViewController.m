...
- (void)viewDidLoad {
	...
	if (isPromotionFirstLoad) {
		[self accessService];
		isPromotionFirstLoad = NO;
	}else {
		[self update];
	}
...
-(void)accessService{
	NSDictionary *jsontestDic = [NSDictionary dictionaryWithObjectsAndKeys:[Common getVersion:PROMOTIONS_ID],@"ver",[NSNumber numberWithInt: shop_id],@"shop-id",[NSNumber numberWithInt: site_id],@"site-id",nil];
	NSString *reqStr = [Common TransformJson:jsontestDic withLinkStr:[ACCESS_SERVICE_LINK stringByAppendingString:@"/hot/pre.do?param=%@"]];
	CommandOperation *commandOpertmp = [[CommandOperation alloc]initWithReqStr:reqStr command:PROMOTIONS delegate:self];
...
-(void)update{
	...
	self.promotionList = (NSMutableArray *)[DBOperate queryData:T_PROMOTIONS theColumn:nil theColumnValue:nil withAll:YES];
	if (self.progressHUD) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	NSArray *hot = [promotionList objectAtIndex:[indexPath row]];
	NSNumber *isread = [hot objectAtIndex:promotions_isread];
	NSNumber *isrecommend = [hot objectAtIndex:promotions_is_recommend];
    if (cell == nil) {
	...
	NSString *piclink = [hot objectAtIndex:promotions_pic];
	if (piclink.length > 1) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	UIImageView *news = [cell.contentView viewWithTag:1000];
	UIImage *img = [[UIImage alloc]initWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"没有更新" ofType:@"png"]];
	news.image = img;
	
	NSArray *promotion = [promotionList objectAtIndex:[indexPath row]];	
	NSNumber *promotionId = [promotion objectAtIndex:promotions_id];
	[DBOperate updateData:T_PROMOTIONS tableColumn: @"isread" columnValue: @"1" conditionColumn:@"id" conditionColumnValue:promotionId];
	browserViewController *browser = [[browserViewController alloc]init];
	browser.isHideToolbar = NO;
    browser.linkurl = [promotion objectAtIndex:promotions_url];
	browser.linktitle = [promotion objectAtIndex:promotions_pic_name];
...
-(UIImage*)getPhoto:(NSIndexPath *)indexPath{
	NSLog(@"getPhoto");
	NSArray *one = [promotionList objectAtIndex:[indexPath row]];
    NSString *url = [one objectAtIndex:promotions_pic];
...
-(NSString*)getPhotoURL:(NSIndexPath *)indexPath{
	NSArray *hot = [promotionList objectAtIndex:[indexPath row]];
	return [hot objectAtIndex:promotions_pic];
}
