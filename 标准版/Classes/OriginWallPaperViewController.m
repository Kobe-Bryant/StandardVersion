...
@implementation OriginWallPaperViewController
...
- (void)viewDidLoad {
	...
	[tbitmp release];
//	self.toolBar.tintColor = [UIColor colorWithRed:BTO_COLOR_RED green:BTO_COLOR_GREEN blue:BTO_COLOR_BLUE alpha:1.0];
...
- (void) showCustomToolbar{
	...
	toolBarView = [[UIView alloc] initWithFrame:CGRectMake(0, yValue, 320, 44)];
	toolBarView.tag = 3005;
	[self.view addSubview:toolBarView];
	
	UIToolbar *myToolBar = [[UIToolbar alloc] initWithFrame:
							CGRectMake(0.0f, 0, 320.0f, 44.0f)];
    if (IOS_VERSION >= 7.0) {
	...
	[toolBarView addSubview:myToolBar];
	...
	UIBarButtonItem   *freshButton=[[UIBarButtonItem alloc]  initWithTitle: @"保存到本地" style: UIBarButtonItemStyleBordered target:self   action:@selector(HandleLoadDown:)];
	...
	UIBarButtonItem   *searchSelfButton=[[UIBarButtonItem alloc] initWithTitle: @"分享" style: UIBarButtonItemStyleBordered target:self   action:@selector(HandleShare:)];
...
- (void)showToolBar{
	...
	UIToolbar *myToolBar = [[UIToolbar alloc] initWithFrame:  
							CGRectMake(0.0f, yValue, 320.0f, 44.0f)];
...
//		NSString *domain=[dic objectForKey:@"domain"];
		...
		NSArray *wp = [picArray objectAtIndex:pageControl.currentPage];
		NSString *linkurl = [wp objectAtIndex:wallpaper_pic_url];
		NSString *linktitle = [wp objectAtIndex:wallpaper_title];
...
-(void)touchOnce{
    ...
    toolBarView.hidden = !navBarState;
}
...
- (void) scrollViewDidScroll: (UIScrollView *) aScrollView
{
	...
	if(aScrollView.tag == 1000){
		...
		if(tString != nil){
			...
			self.title = [NSString stringWithFormat:@"(%d/%d)",pageControl.currentPage+1,[picArray count]];
		}
